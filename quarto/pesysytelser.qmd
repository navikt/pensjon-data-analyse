...
map_ytelser = {
    ...
    "afp_privat": "Alderspensjon + AFP Privat",
}
df_pesys = df_pesys.rename(columns=map_ytelser)

# reduser antallet i "Alderspensjon" med antallet i "Alderspensjon + AFP Privat"
df_pesys["Alderspensjon"] = df_pesys["Alderspensjon"] - df_pesys["Alderspensjon + AFP Privat"]

df_ufore = pd.merge(
    df_pesys[["mottaker_alder", "Uføretrygd"]],
...
df_ufore["prosent uføre"] = ((df_ufore["Uføretrygd"] / df_ufore["befolkning"]) * 100).round(0).fillna(0).astype(int)
...
ytelser = ["Uføretrygd", "Gjenlevendeytelse", "Alderspensjon", "Alderspensjon + AFP Privat", "AFP"]
...
fig_ufore.add_trace(go.Bar(
    ...
    name="Uføre",
    customdata=df_ufore["prosent uføre"],
    hovertemplate=f"Antall med uførevedtak: %{{y:}}<br>Prosent uføre av %{{x}}-åringer: %{{customdata}} %<extra></extra>",
...
#| icon: people
#| color: primary
f"{df_ufore['Uføretrygd'].sum():,.0f}".replace(',', ' ')
```
### col
```{python}
...
#| icon: people
#| color: primary
f"{round((df_ufore[df_ufore["mottaker_alder"].between(18, 67)]["Uføretrygd"].sum() / df_ufore[df_ufore["mottaker_alder"].between(18, 66)]["befolkning"].sum())* 100, 1):,.1f} %".replace('.', ',')
```
### col
```{python}
...
#| icon: people
#| color: primary
f"{round((df_ufore["Uføretrygd"].sum() / df_ufore["befolkning"].sum())* 100, 1):,.1f} %".replace('.', ',')
```
...
fig_alderspensjon.data[2].visible = "legendonly"
...
# Tall fra offentlig statistikknotat
...
<!-- ### col
::: {.valuebox color="primary"  icon="graph-up"}
Økning i andel av befolkningen 18-67 år

0,1%

fordi befolkningen i aldersgruppen også økte
::: -->
...
# Tall fra offentlig statistikknotat-2
## Rad øverst med tekst
Dette er tall fra Navs offentlige årsstatistikknotat for uføretrygd i 2024. [Tilgjengelige på nav.no](https://www.nav.no/no/nav-og-samfunn/statistikk/aap-nedsatt-arbeidsevne-og-uforetrygd-statistikk/uforetrygd/uforetrygd-statistikknotater)
