...
secret_name = "projects/230094999443/secrets/DWHI/versions/latest"
client = secretmanager.SecretManagerServiceClient()
response = client.access_secret_version(request={"name": secret_name})
secret = json.loads(response.payload.data.decode("UTF-8"))

connectionDWHI = oracledb.connect(
...
logging.info(f"Koblet til DWHI med bruker: {secret.get('DB_USER')}")

sql = "select * from dt_p.mv_ufore_aldersgruppe order by aar_maaned desc"
with connectionDWHI.cursor() as cursor:
    cursor.execute(sql)
    columns = [col[0].lower() for col in cursor.description]
    data = cursor.fetchall()
connectionDWHI.close()
