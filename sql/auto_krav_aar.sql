select sakstype, k_behandling_t as automatisering, kravtype, extract(year from dato_opprettet) aar, batch, count(*) antall
from (
select
d_s.dekode sakstype
...
--and kh.dato_opprettet >= to_Date('01.01.2022','DD.MM.YYYY')
--and kh.k_krav_gjelder in ('REGULERING','TILBAKEKR','OVERF_OMSGSP','ENDR_UTTAKSGRAD','INNT_E','FORSTEG_BH','REVURD', 'UT_EO')
) A

group by sakstype, k_behandling_t, kravtype, extract(year from dato_opprettet), batch
