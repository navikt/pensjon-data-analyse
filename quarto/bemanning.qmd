...
def po_oversikt(po: str):
    ...
    fig.update_layout(
        ...
        annotations=[
            dict(text="Kjønn", x=0.1, y=0.1, font_size=14, showarrow=False),
            dict(text="Aldersgruppe", x=0.5, y=0.1, font_size=14, showarrow=False),
            dict(text="Ansettelsestype", x=0.9, y=0.1, font_size=14, showarrow=False)
        ]
    )
    return fig
...
## {height=20%}
```{python}
po_oversikt("PO pensjon")
```

```{python}
po_oversikt('PO arbeidsgiver')
```
## {height=20%}
```{python}
po_oversikt('PO utbetaling')
```
```{python}
po_oversikt('PO helse')
```
## {height=20%}
```{python}
po_oversikt('PO familie')
```
```{python}
po_oversikt('PO arbeidsoppfølging')
```
## {height=20%}
```{python}
po_oversikt('PO arbeid')
```
```{python}
po_oversikt('PO AAP')
```
...
def create_pensjon_bar_chart(group_by_column: str):
    ...
    df_pensjon_grouped = df_pensjon_grouped.sort_values(by=["andel_text", "team"], ascending=[False, False])

    fig = go.Figure()
    for value in df_pensjon_grouped[group_by_column].unique()[::-1]:
...
```{python}
create_pensjon_bar_chart("kjonn")
```

```{python}
create_pensjon_bar_chart("aldersgruppe")
```
